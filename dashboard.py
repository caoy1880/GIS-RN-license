...
def load_excel_data(file_path):
    ...
    for year in YEARS:
        ...
        df = df[["County", "Nurses per 10,000 Population"]].copy()
        df["Year"] = year
        all_data.append(df)
...
grouped = (
    df_year.groupby("County", as_index=False)
           .agg({"Nurses per 10,000 Population": "mean"})  # or "first" if no duplicates
)
...
def build_hover(row):
    return (
        ...
        f"Nurses per 10,000 Population: {row['Nurses per 10,000 Population'] if pd.notna(row['Nurses per 10,000 Population']) else 'N/A'}"
    )
...
# ========== PLOTLY MAP ==========
nurses = merged["Nurses per 10,000 Population"]
...
fig = go.Figure(go.Choropleth(
    ...
    colorbar_title="Nurses per 10,000 Population",
))
